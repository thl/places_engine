module PlacesEngine
  ...
  module FeaturesControllerExtensions
    ...
    def fids_by_name
      ...
      @features = perform_global_search(search).where(conditions).joins(:shapes).references(:shapes)
      @features = @features.joins(joins.join(' ')) unless joins.empty?
      respond_to do |format|
    ...
    def topics
      ...
      if @feature.nil?
        ...
        respond_to do |format|
          ...
          format.xml
          format.js
          format.json { render json: Hash.from_xml(render_to_string(action: 'topics', formats: [:xml])) }
        end
      end
    end
  end
