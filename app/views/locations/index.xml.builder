  ...
  xml.altitudes(:type => 'array') do
    @feature.altitudes.each do |altitude|
      xml.altitude do
        ...
        xml << render(partial: 'time_units/index', formats: [:xml], locals: {time_units: altitude.time_units})
        xml << render(partial: 'citations/index', formats: [:xml], locals: {citations: altitude.citations})
        xml << render(partial: 'notes/index', formats: [:xml], locals: {notes: altitude.notes})
      end
    end
  end
  ...
  xml.shapes(:type => 'array') do
    shapes.each do |shape|
      xml.shape do
        ...
        xml << render(partial: 'time_units/index', formats: [:xml], locals: {time_units: shape.time_units})
        xml << render(partial: 'citations/index', formats: [:xml], locals: {citations: shape.citations})
        xml << render(partial: 'notes/index', formats: [:xml], locals: {notes: shape.notes})
      end
    end
  end
  if !shapes.empty?
