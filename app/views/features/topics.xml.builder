xml.instruct!
xml.feature do
  xml << render(partial: 'feature_types', formats: [:xml], object: @feature.feature_object_types)
    ...
    @feature.category_features.order(:position).each do |association| 
      if !association.instance_of? FeatureObjectType
        xml.category_feature do
          ...
          xml << render(partial: 'time_units/index', formats: [:xml], locals: {time_units: association.time_units})
          xml << render(partial: 'citations/index', formats: [:xml], locals: {citations: association.citations})
          xml << render(partial: 'notes/index', formats: [:xml], locals: {notes: association.notes})
        end
      end
    end
  end
